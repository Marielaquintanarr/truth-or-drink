...
export default function DrinkPage() {
  ...
  const [currentDrink, setCurrentDrink] = useState<string>("Loading...");
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // 🔥 Selecciona un drink inicial solo una vez cuando cargan los datos
        if (data.length > 0) {
          const randomDrink =
            data[Math.floor(Math.random() * data.length)].drink;
  ...
  // 🔥 Cambia el drink solo cuando el jugador presiona "Next"
  const handleNext = () => {
    ...
    <>
      <div className="flex flex-col items-center justify-center min-h-screen">
        ...
        <Link to="/resultspage" className="absolute top-4 left-4 z-50">
          <img
            className="cursor-pointer hover:opacity-80 w-12 h-12"
            src={x}
            alt="Exit"
          />
        </Link>
